...
describe("support triage contracts", () => {
  it("validates a well-formed ticket payload", () => {
    const ticket = {
      ...
      message:
        "Our admin users cannot access the dashboard since the latest deployment.",
  ...
  it("builds prompt pack with required JSON instructions", () => {
    ...
    expect(prompt.system).toContain("Base priority on the NATURE of the failure");
  ...
  it("validates a well-formed triage result", () => {
    const result = {
      ...
      autoResponse:
        "Thank you for reaching out. Our billing team will review this shortly.",
  ...
  it("accepts optional zendesk payload in triage result", () => {
    const result = {
      ...
      autoResponse:
        "Our engineering team has been alerted and is investigating.",
      shouldAutoRespond: false,
      zendeskData: {
        ticketId: "123456",
        status: "open",
